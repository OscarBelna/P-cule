 ...
 */
export function lightenColor(hex, factor = 0.6) {
...
/**
 * Crée la configuration complète pour un graphique en camembert
 * @param {string[]} labels - Les labels des catégories
 * @param {number[]} values - Les valeurs des catégories
 * @param {string[]} colors - Les couleurs de fond (éclaircies)
 * @param {string[]} borderColors - Les couleurs de bordure
 * @param {formatCurrency} formatCurrency - Fonction de formatage de la devise
 * @returns {Object} Configuration Chart.js pour un graphique en camembert
 */
export function createDoughnutChartConfig(labels, values, colors, borderColors, formatCurrency) {
    return {
        type: 'doughnut',
        data: {
            labels: labels,
